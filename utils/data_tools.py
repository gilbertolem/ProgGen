...
def musicxml2onehot(xml_directory, words_text2num, filters = {'author':None, 'style':None}):
    ...
    tunes_tensor = torch.zeros(max_len, len(tunes_train), len(words_text2num))
    tunes_tensor[:,:,0] = torch.ones(max_len, len(tunes_train))       # Initialize all the tensor with first word (blank)
    ...
    # Create and fill tensor (Sequence x Batch x Feature)
    val_tensor = torch.zeros(max_len, len(tunes_val), len(words_text2num))
    val_tensor[:,:,0] = torch.ones(max_len, len(tunes_val))       # Initialize all the tensor with first word (blank)
    for i, tune in enumerate(tunes_val):
        matrix = torch.tensor(tune.matrix_form(words_text2num))
        val_tensor[0:len(matrix), i, :] = matrix
